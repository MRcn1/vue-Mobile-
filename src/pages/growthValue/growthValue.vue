<template>
  <div class="scoreLog">
    <div class="tip">最近3个月的成长值记录</div>
    <div class="list">
      <div class="listItem" v-for="(item,index) in data" :key="index">
        <div class="m">
          <span class="x">{{item.createdDate}}</span><br><span class="time">{{item.recordType}}</span>
        </div>
        <div class="r">
          <span class="jia" v-if="item.rate>=0">+{{item.rate}}</span>
          <span class="jian" v-if="item.rate<0">{{item.rate}}</span>
        </div>
      </div>
    </div>
    <pages url='/membershipCardSalesRateRecord/findPageByMembershipCardId' :tab='false' :receiveUrl='receiveUrl' :pageSize='pagesize' :obj='obj' ref="page"></pages>
